...
public class AsyncImageDecoder
{
    ...
    // Whether need update the file modify date
     ...
     * @param listener callbacks while image decoded.
     */
    public AsyncImageDecoder(Context context, IImageDecodeListener listener)
    {
        mContext = context;
        mFilter = false;
        mScale = false;
     ...
     * @param width desired bitmap width.
     * @param height desired bitmap height.
     * @param filter boolean used in {@link createScaledBitmap}.
     */
    public AsyncImageDecoder(Context context, IImageDecodeListener listener,
     ...
     * @param height desired bitmap height.
     * @param filter boolean used in {@link createScaledBitmap}.
     * @param touchFile set file's last modified time after bitmaps decode
     *            succeed.
     */
    public AsyncImageDecoder(Context context, IImageDecodeListener listener,
            int width, int height, boolean filter, boolean touchFile)
    {
        mContext = context;
        mWidth = width;
        mHeight = height;
    ...
    /**
     * Sets visible range when it changed. Images in visible range will decode
     * first.
     * 
     * @param start
     * @param end
     ...
     * 
     * @param key the index will be return in {@link onImageDecoded}
     * @param path the path of the image.
     * @param position used in getting visible items
     * @param varargs the argument will be return in {@link onImageDecoded}
     */
     ...
     * @param path the path of the image.
     * @param position used in getting visible items
     * @param notScale force decode with original size if not exceed MAX size
     * @param varargs the argument will be return in {@link onImageDecoded}
     */
     ...
     * 
     * @param key the index will be return in {@link onImageDecoded}
     * @param albumId the album id of the image
     * @param position used in getting visible items
     * @param varargs the argument will be return in {@link onImageDecoded}
     */
     ...
     * 
     * @param key the index will be return in {@link onImageDecoded}
     * @param afd The file descriptor containing the bitmap data to decode
     * @param position used in getting visible items
     * @param varargs the argument will be return in {@link onImageDecoded}
     */
     ...
     * 
     * @param albumId the album id in database
    ...
    private String getPathByAlbumId(int albumId)
    {
        ...
        Cursor mCursor = resolver.query(uri, new String[] {
                "_data"
        }, null, null, null);
        ...
        if (uri != null)
        {
            ...
            try
            {
                ...
                if (mScale)
                {
                    ...
                    BitmapFactory
                            .decodeFileDescriptor(fd.getFileDescriptor(), null, mBitmapOptions);
                ...
                Bitmap bmp = BitmapFactory.decodeFileDescriptor(fd.getFileDescriptor(), null,
                        mBitmapOptions);
    ...
    private Bitmap decodeByFileDescriptor(AssetFileDescriptor assetFileDes)
    {
        ...
        try
        {
            ...
            if (mScale)
            {
                mBitmapOptions.inSampleSize = 1;
                mBitmapOptions.inJustDecodeBounds = true;
                BitmapFactory.decodeFileDescriptor(assetFileDes.getFileDescriptor(), null,
                        mBitmapOptions);
                int nextWidth = mBitmapOptions.outWidth >> 1;
                int nextHeight = mBitmapOptions.outHeight >> 1;
                while (nextWidth > mWidth && nextHeight > mHeight)
            ...
            mBitmapOptions.inSampleSize = sampleSize;
            mBitmapOptions.inJustDecodeBounds = false;
            Bitmap bmp = BitmapFactory.decodeFileDescriptor(assetFileDes.getFileDescriptor(), null,
                    mBitmapOptions);
            if (bmp != null && mScale)
            {
                if (mBitmapOptions.outWidth != mWidth || mBitmapOptions.outHeight != mHeight)
    ...
    private Bitmap decodeByURL(String UriPath)
    {
        ...
        try
        {
            ...
            Log.d(TAG,
                    "[decodeByURL] Scaled : width=" + bmp.getWidth() + ",height=" + bmp.getHeight());
                ...
                { // In order to make call back on UI thread
                    ...
                    if (task != null && (null != task.mBitmap || mReportError))
                    {
                        if (mListener != null)
                        {
                            ...
                            mListener.onImageDecoded(task.mKey, task.mBitmap, task.mTimeStamp,
                                    task.mVarargs);
                        }
                        else
                        {
                            Log.e(TAG,
                                    "[handleMessage]onImageDecoded mListener == null, recycle bitmap");
        ...
        private void doDecode()
        {
            ...
            if (task != null)
            {
                ...
                if (bitmap != null)
                {
                    ...
                    if (mTouchFile == true && task != null && task.mPath != null
                            && task.mPath.length() > 0)
                    {
    ...
    public interface IImageDecodeListener
    {
         ...
         * 
         * @param position the first parameter in add function
         * @param bitmap the decoded bitmap
         * @param timeStamp the request time of this bitmap
         * @param arg0 the argument of this decoded image
         */
